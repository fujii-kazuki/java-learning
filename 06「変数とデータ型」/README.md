...
- [キャスト演算子と型変換のルール](#キャスト演算子と型変換のルール)
  - [自動的な型変換（拡大変換）](#自動的な型変換拡大変換)
  - [intやlongからfloatやdoubleへ変換するときの注意点](#intやlongからfloatやdoubleへ変換するときの注意点)
  - [キャスト演算子を使用した縮小変換](#キャスト演算子を使用した縮小変換)
  - [longからintやdoubleからintへ変換するときの注意点](#longからintやdoubleからintへ変換するときの注意点)
  - [数値リテラルを代入する場合の自動変換](#数値リテラルを代入する場合の自動変換)
- [ラッパークラス](#ラッパークラス)
  - [ラッパークラスのオブジェクト生成](#ラッパークラスのオブジェクト生成)
  - [ラッパークラスの値を取得](#ラッパークラスの値を取得)
  - [オートボクシング](#オートボクシング)
...
## ラッパークラス
基本データ型の値を持ち、様々なメソッドを用意したクラスが用意されているクラスの事。  
基本データ型に対応したラッパークラスのオブジェクトを作成し、そのオブジェクトに対して用意されたメソッドを使用すれば数値から文字列へ変換といった処理が可能となる。  

基本データ型ごとに対応するラッパークラスは次のとおり。
| 基本データ型 | ラッパークラス |
| --- | --- |
| boolean | Boolean |
| char | Character |
...
### ラッパークラスのオブジェクト生成
例として、int型の値を持つIntegerクラスのインスタンスを作成。  
インスタンスを作成するにはIntegerクラスで用意されている`valueOf`メソッドを使用する。
```
Integer i = Integer.valueOf(16);
```
Integerクラスのコンストラクタ[^2]を利用したインスタンス生成も可能だが、この方法は**非推奨**となっている。  
コンパイル時に推奨されない方法として警告が表示される。
```
Integer i = new Integer(16);  //非推奨
```

### ラッパークラスの値を取得
例として、Integerクラスのオブジェクトから値を取得するにはIntegerクラスで用意されている`intValue`メソッドを使用する。
```
Integer i = Integer.valueOf(16);
int val = i.intValue();
System.out.println("val = " + val);   //val = 16
```
他のラッパークラスも同じ様なメソッドがそれぞれ用意されている。

### オートボクシング
ラッパークラスのインスタンスを記述すべきところにラッパークラスに対応した基本データ型の値を記述すると、自動で変換されるというもの。

```
Integer i = Integer.valueOf(16);
```
これを`オートボクシング`の機能を利用して記述すると
```
Integer i = 16;
```

また、値を取得する際にも`オートボクシング`を利用できる。
```
Integer i = Integer.valueOf(16);
int val = i.intValue();
```
...
`オートボクシング`を利用することでラッパークラスのオブジェクトをあたかも基本データ型のように扱うことができる。


[^1]: あらかじめ用途が決められた単語のことで、それ以外の意味では使用することができない。一例として`byte`や`class`などが挙げられる。
[^2]: インスタンスを作ったタイミングで実行されるメソッドのこと。